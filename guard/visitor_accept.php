...
for ($i = 0; $i < $count; $i++) {
	...
	if (empty($row[1])) {
		...
		$destination = mysqli_query($conn, "REPLACE INTO `qr_users` (`id`, `qr_firstname`, `qr_lastname`, `qr_number`, `qr_gender`, `qr_purpose`, `qr_pin`, `qr_time_in`) VALUES ('$i','$firstname','$lastname','$number','$gender','$purpose', '$pin', '$time_in')");
		if ($destination) {
